...
var ingestStatusServiceTracer = otel.Tracer("ingest_status_service")

// IngestStatusService handles business logic for ingestion operations
type IngestStatusService struct {
	ingestRepo *repositories.IngestRepository
}

// NewIngestStatusService creates a new IngestStatusService instance
func NewIngestStatusService(ingestRepo *repositories.IngestRepository) *IngestStatusService {
	return &IngestStatusService{
		ingestRepo: ingestRepo,
	}
}

// GetIngestStatus retrieves current and recent ingestion status for a user
func (s *IngestStatusService) GetIngestStatus(ctx context.Context, userID uuid.UUID, limit int) (*dto.IngestStatusDTO, error) {
	ctx, span := ingestStatusServiceTracer.Start(ctx, "GetIngestStatus")
	defer span.End()
